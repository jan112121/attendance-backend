...
export const verifyAttendance = async (req, res) => {
  try {
    ...
    // Only allow scanning between 6AM–6PM
    if (hours < 6 || hours >= 18) {
    ...
    // 🔞 Determine session & late thresholds
    let session = '', lateThreshold = 0, sessionEnd = 0;
    if (hours >= 6 && hours < 12) {
      session = 'morning';
      lateThreshold = 7;       // 7 AM
      sessionEnd = 11.5;       // 11:30 AM
    } else {
      session = 'afternoon';
      lateThreshold = 13;      // 1 PM
      sessionEnd = 17.5;       // 5:30 PM
    }
    ...
    if (!attendance) {
      // ⏰ Time-in
      let status = 'present';
      if (hours >= lateThreshold) status = 'late';
